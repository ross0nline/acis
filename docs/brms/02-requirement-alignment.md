...
Each document is scored by Claude: `risk_level` (High/Medium/Low), `impacted_field` (RxDC/GagClause/HIPAA/GeneralSecurity), `summary`, `remediation_step`, and `deadline`. The Live Pulse panel surfaces the 50 most recent of {{REGULATORY_EVENT_COUNT}} scored documents from a continuously growing database.
